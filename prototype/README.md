...
**Milko's report that started it:** with the `2cdf3eb` build, `?autoplay=1&kill_bar=6&kill_count=20` and `?probe=1` (private tab) did NOT kill the tab: no reload, no "a problem repeatedly occurred", no red LAST SESSION lines after a reload. Instead they "glitch": the screen jumps all over the place, the character is invisible, the music keeps playing. A normal run from the menu with 6–8 deaths (RETRY, MENU → Start Game, every button) was fine. **Open question: is the glitch the 20 forced rewinds working as designed (a rewind every death, the creature GONE until each respawn), or Mac WebKit's death storm (context lost, the song no longer following seeks, a death every 0.28 s at the respawn point)?** The two builds below exist to answer that from phone data.

**1. The black box phones home (`f667816`).** `tools/serve.py` has `POST /bb?s=<session>`: every line the game's black box records is now ALSO sent with `navigator.sendBeacon` (a beacon is queued by the browser and delivered even while the page is going away) and appended to **`../the-last-game-build/blackbox.log`** with the Mac's clock and a session id — `tail -f ../the-last-game-build/blackbox.log` while the phone plays. localStorage stays as before. New in the record: **`webglcontextlost` / `webglcontextrestored`** on the canvas, uncaught JS errors, the tab going hidden/visible, `pagehide` (a clean exit — its absence after a session is the kill); **every death carries the clock against the song** (`clock 26.32 (local 26.32) heard 25.10 drift -1219 ms (base -837)`: `heard` is where the song really is, `BeatClock.heard_time()`), every preroll and seek too, and **0.5 s and 2 s after every seek** a line from `_process` says whether the song followed (`0.5s after seek: clock 8.42 heard 7.67`, `2.0s after seek: clock 9.92 heard 8.94` — Chrome on the Mac: the reported position lags ~1 s after a seek, then moves); the heartbeat carries the drift and **the window size**, and a `resize WxH` line is written whenever it changes (the rotation question). Every line's time is page time (seconds since the page was opened). Nothing inside a frame callback. **Proof:** headless bot `kill_bar=4 kill_count=3` — three deaths, three rewinds, no script error; layout hash level 1 unchanged (`4db8f95b…`); headless Chrome against the served build: every line reached the log, pagehide included (`/private/tmp/.../scratchpad/bb_test.py`, a 20-line Playwright script: open the link, wait for the deaths, close the page).

**2. The boot line (`db5683e`).** One `boot` line per session from the browser's own clock: the HTML's arrival, `index.js` / `index.wasm` / `index.pck` start–end with bytes over the wire (Resource Timing), the wasm instantiate span (`window.pr_wasm`, a wrapper in the preset's `head_include` around `WebAssembly.instantiate` and `instantiateStreaming` — the shell streams, so download and compile overlap; the compile tail is its end minus the wasm's end), and when Godot's main started; then `first frame painted`, `load music/validate/nodes/prewarm` on the same clock. **The Mac, Chrome, localhost: html 0.04 s · wasm 37.7 MB 0.1–0.3 s · pck 26.5 MB 0.1–0.3 s · instantiate 0.1–0.3 s · engine main 0.4 s · first frame 1.5 s · prewarm 1.6 s.** The phone's line is what section 2's table is built from; **no cut is made yet.** (One thing to know before reading it: the phone is on the Mac's hotspot link, `172.20.10.x`, and the server sends the 37.7 MB wasm and the 26.5 MB pck uncompressed.)
...
Then the next session reads `../the-last-game-build/blackbox.log` (each phone session is a block under `--- session from 172.20.10.1`) and answers: the glitch's nature (death cadence and causes; `heard` after each seek; any `WEBGL CONTEXT LOST`), the loading table (section 2), the rotation (`win`).
...
### 2026-09-24 (night) — THE CRASH HUNT: the black box, what Mac WebKit showed, four commits, exported, served

**The crash is NOT solved.** Milko's report: with the shared-song-buffer build (`5ae1b75`) iOS Safari still kills the tab on `?probe=1` and on `?autoplay=1&kill_bar=6&kill_count=20`, at the first rewind (about 66 m), also in a private tab. What was done tonight, in order:

**1. The black box (`0bd170f`, `prototype/blackbox.gd`).** The last 30 events go to the browser's localStorage as they happen: the loading steps, every preroll and seek, every death with its cause, the probe's setups, and a heartbeat every 2 s (bar · frame ms · deaths · seeks · texture and buffer memory · object count). A session marks itself open at start and closed on `pagehide`; if the next load finds the mark still open, the readout shows the previous session's last lines in red under **LAST SESSION ENDED AT:** (also printed as `BLACK BOX, last session:`). Web only. **This is on the served build now: reproduce the crash on the phone, reload, read the red lines.**

**2. What Mac WebKit showed — and what it is not.** Playwright's headed WebKit build on the Mac (real WebGL, ANGLE Metal — not the headless one, which has no WebGL) reproduced *a* crash: the WebKit GPU process segfaults in ANGLE's buffer bookkeeping (`gl::Buffer::getMemorySize` ← `RemoteGraphicsContextGL::updateMemoryCost`; five macOS crash reports in `~/Library/Logs/DiagnosticReports/`), at 280–400 MB — **not a memory kill.** On the Mac the GPU process restarts and the page limps on ("object does not belong to this context" warnings, and the song stops following seeks, so the bot dies every 0.28 s at the respawn point: that "death storm" is a symptom, not a cause). **But it did NOT crash where the phone does:** a plain bot with no death at all crashed 5.7–7.9 s after page load (bar 0–1, before any rewind), in every variant tried (finishing layer off, pillars off, light off, half render scale); the 20-death link crashed at 11 s once and at 78–84 s (after three rewinds) twice; the menu alone survived. So this is Playwright's WebKit, and it is not proven to be the phone's crash — Milko's call, 2026-09-24: **no more Playwright WebKit; real Safari with the Web Inspector is the next measurement,** and it needs one thing from him first (see NEXT).

**3. What the hunt found anyway, and fixed — real churn, worth removing, not proven to be the phone's crash:**
- **`90b7506` The touch controls and the HUD stopped rebuilding their shapes every frame.** Counted from inside the page: about **2,800 WebGL buffer deletes and 1,850 vertex-array deletes a second** during plain play with nobody dying — every `draw_circle` / `draw_arc` of `ui/ui.gd` (24 a frame) and `hud.gd` is a polygon, a GPU buffer built for the frame and deleted the next. The same shapes and numbers are now painted once into four child layers, moved and faded (`modulate.a`) each frame; the JUMP layer repaints only during its flash, the HUD only when a value changed. Native before/after screenshots at bar 1: the corners are the same picture. **After this, steady play deletes 0 buffers per 5 s** (a jump's flash still repaints for a quarter second: ~225 per jump).
- **`dfce8b0` The footfall dust:** two emitters of fixed size instead of one whose `amount` was set per puff (setting it reallocates the GPU buffer, every step).
- **`4f76ae3` One mesh per size** (`Mats.box`, `Mats.sphere` in `flat_mats.gd`): tiles, slabs, checkpoints, notes, gate posts and hazard bars shared their geometry — about 16,500 WebGL buffers created while a run loaded, and a lap's worth deleted in one frame each time a lap was passed. Layout hash level 1 unchanged, the bot unchanged. This build has run in no WebKit at all (Milko stopped that); the headless bot and the hash are its proof.

**4. Not changed, on record:** `BeatClock` prerolls at death and the rewind only unmutes (the death spike fix, `578945b`); the song is one shared buffer (`452253c`); the prewarm rack is still freed after loading (a handful of buffers, not the thousands it was).
...
### NEXT, in this order (2026-09-24, night)

0. **RUN THE THREE LINKS ABOVE ON THE PHONE**, then read `../the-last-game-build/blackbox.log` — the glitch's nature, the loading table (section 2 of the brief of 2026-09-24 late: report as a table with suggested cuts, don't make them), the rotation. Nothing below in this item is needed if the log answers it.

0. **THE iOS CRASH, first.** (a) Milko, on the phone, with the served build: open `https://172.20.10.2:8443/?autoplay=1&kill_bar=6&kill_count=20`, let it crash, reload the same link and read the red **LAST SESSION ENDED AT** lines (or `?probe=1` the same way; a normal run from the menu too, if it dies) — the last heartbeat's bar, frame ms, `tex`/`buf` MB and `objs`, and whether the last line is a `preroll`, a `seek`, a `death` or a `probe` setup, is what the next session works from. (b) **Real Safari on the Mac with the Web Inspector** (the one exception to rule 4) against the same two links, memory timeline and console; `safaridriver` is already enabled on this Mac (checked: it answers `ready`), so a script can drive REAL Safari through WebDriver; the one hurdle is the self-signed certificate's warning page, which no script may click through — Milko opens `https://172.20.10.2:8443` once in Safari on the Mac and accepts it (or trusts the certificate in Keychain), and the next session measures in real Safari from there. (c) The candidates still open: the decoded song buffer (stream it through an `<audio>` element if the shell allows), textures unpacked to RGBA on iOS (the `tex` word on the readout: no `etc2` = unpacked), the wasm heap size, and anything else that allocates per death or seek. Until (a) or (b) says otherwise, nothing above is proven to be the phone's crash.
