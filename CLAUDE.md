...
**Read `PHASE_R_BRIEF.md`, then `PHASE_R_ADDENDUM_1.md`, then `PHASE_R_ADDENDUM_2.md`; they supersede the roadmap below while Phase R runs.** The synthwave art direction is dropped; the game becomes a music-driven precision game where each level is a song. The 2D game is untouched and still the project's main scene. Everything new lives in `prototype/` (see `prototype/README.md` for the run command, the file map and the tuning knobs). Status (2026-09-15): addendum 2 built — 18-wide field, high camera, level 1 as a taught curriculum with demo bars, no lives on level 1, progress bar, death line. Verification is headless only: `rules.gd` decides every death and logs it, `fairness.gd` validates the layout, `tools/autoplay.gd` has a validator-path bot (must never die) and a human-like bot (the "five deaths" acceptance). Milko does the on-device feel test himself; never open a window on his Mac to "play". Web preset "Web (Phase R)" exports to `build/phase-r/`; `tools/package_web.sh "Web (Phase R)"` then `tools/serve.py tls build/phase-r`.
